...
class TilapiaLakeVirusInformationScreen extends StatefulWidget {
  ...
  _TilapiaLakeVirusInformationScreenState createState() =>
      _TilapiaLakeVirusInformationScreenState();
}

class _TilapiaLakeVirusInformationScreenState
    extends State<TilapiaLakeVirusInformationScreen> with SingleTickerProviderStateMixin {
  String? localPath;
  bool isLoading = true;
  int? totalPages;
  int? currentPage;
  ...
  Future<void> loadPDF() async {
    try {
      final bytes = await rootBundle.load('lib/assets/pdfs/tilapia_virus.pdf');
      final dir = await getTemporaryDirectory();
      final file = File('${dir.path}/tilapia_virus.pdf');
  ...
  Future<void> _sharePDF() async {
    if (localPath != null) {
      await Share.shareXFiles([XFile(localPath!)],
        subject: 'Tilapia Lake Virus Information',
      );
    }
  }
                ...
                children: [
                  CircularProgressIndicator(
                    valueColor: _loadingController.drive(
                      ColorTween(
                        begin: const Color(0xFF40C4FF),
                        end: const Color(0xFF0091EA),
                      ),
                    ),
                  ),
